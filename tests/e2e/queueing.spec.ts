...
test.describe('queueing', () => {
  ...
  // Drag/tap add melody:brass via clicks (tap-select + tap-part)
    ...
    await page.waitForFunction(({ id }) => {
      ...
      return expected && st && (typeof st.soft === 'number' || typeof st.loud === 'number') && Math.abs((st.soft ?? st.loud!) - expected) < 0.5
    }, info, { timeout: 15000 })
  ...
  // within ~500ms of scheduled time is acceptable in CI
  expect(Math.abs(minStart - info.scheduleTime)).toBeLessThan(0.5)
  })
})
